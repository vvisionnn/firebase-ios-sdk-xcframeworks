...
zip_frameworks() {
  for i in */*.xcframework; do (
    local name=$(xcframework_name $i)
    cd "$i/../"
    zip -rqo "$name.xcframework.zip" "$name.xcframework"
  ) & done
  wait
}
...
write_target() {
  ...
  echo "$dependencies" | while read -r dependency; do
    printf ",
        $(
      cd $library
      conditional_dependency $dependency
    )" >>$output
  done
  printf "\n      ]" >>$output
  # Path
...
generate_swift_package() {
  ...
  # Create libraries
  comma=""
  for i in */; do write_library $i $libraries $comma && comma=","; done
  # Create targets that define each library's dependencies and resources
  comma=""
  for i in */; do write_target $i $targets $comma && comma=","; done
  ...
  if [[ -n $local_dist ]]; then
    ...
    comma=""
    for i in $dist/*.xcframework; do write_local_binary $i $local_dist $binaries $comma && comma=","; done
  else
    echo "Creating release Package.swift..."
    # Create remote binary targets, these will be hosted on a github release
    comma=""
    for i in $dist/*.xcframework.zip; do write_binary $i $repo $latest $binaries $comma && comma=","; done
  fi
...
if [[ $latest != $current || $debug ]]; then
  ...
  (
    ...
    (
      cd ..
      swift build
    ) # TODO: create tests and replace this line with `(cd ..; swift test)`
